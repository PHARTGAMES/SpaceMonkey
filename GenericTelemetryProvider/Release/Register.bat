...
reg add "HKLM\SOFTWARE\WOW6432Node\PHARTGAMES\SpaceMonkeyTP" /v "install_path" /t REG_SZ /d "%install_path%\"

:: Add wibblewobble install path registry entry for 64-bit processes
reg add "HKLM\SOFTWARE\PHARTGAMES\SpaceMonkeyTP" /v "install_path" /t REG_SZ /d "%install_path%\"
