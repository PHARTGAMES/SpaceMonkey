...
double SystemTime::GetInMicroseconds()
{
	if (!s_bInitialized)
		Init();

	// Get the current number of ticks relative to the base ticks
	uint64_t nTicks = GetInTicks() - s_nBaseTicks;

	// Convert ticks to seconds, then to microseconds
	// Seconds = ticks * s_flSecondsPerTick
	// Microseconds = seconds * 1,000,000
	double microseconds = (double)nTicks * (1000000.0 / (double)s_nTicksPerSecond);
	return microseconds;
}
