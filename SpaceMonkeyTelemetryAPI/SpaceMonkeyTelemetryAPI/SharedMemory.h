...
public:
	...
	SharedMemType m_type = SharedMemType::SharedMem_Read;
	std::thread* m_thread = NULL;

	SharedMemory(const char* a_memoryName, const char* a_mutexName, SharedMemType a_type, void *a_memoryCopy, int a_memorySize)
	{
		...
		switch (m_type)
		{
			case SharedMemType::SharedMem_Read:
			{
...
//				m_thread = new std::thread(&WWSharedMemory::ThreadRead, this);
			...
			case SharedMemType::SharedMem_Write:
			{
				m_memMap = CreateFileMappingA(INVALID_HANDLE_VALUE,
					NULL,
					PAGE_READWRITE,
...
//				m_thread = new std::thread(&WWSharedMemory::ThreadWrite, this);
			...
			case SharedMemType::SharedMem_ReadWrite:
			{
				m_memMap = CreateFileMappingA(INVALID_HANDLE_VALUE,
					NULL,
					PAGE_READWRITE,
				...
				if (m_memMap == NULL)
				{
					Debug::Log("Failed to open %s for readwrite\n", a_memoryName);
					return;
				}
				...
				if (m_mappedView != NULL)
					Debug::Log("Opened %s for readwrite\n", a_memoryName);
				else
					Debug::Log("Failed to open %s for readwrite\n", a_memoryName);
